...
int main(int argc, char* argv[]) {
    ...
    std::string open_buf;
    InputOption open_opt;
    open_opt.on_enter = [&] {
        if (!open_buf.empty()) {
            std::filesystem::path p = open_buf;
            if (p.is_relative()) {
    ...
    auto open_input = Input(&open_buf, "path/to/file.csv", open_opt);
            ...
            return window(
                ...
                vbox({
                    ...
                    window(
                        text(" Open ") | bold,
                        vbox({
                            hbox({ text("  "), open_input->Render() | flex }),
                            separatorLight(),
                            hbox({ text("  "),
                                   text("Enter") | bold | color(cfg.colors.header),
                                   text("  open    "),
    ...
    auto root = CatchEvent(
        Container::Tab({ main_ui, confirm_dialog, help_dialog, save_confirm_dialog, save_as_dialog, cfg_dialog, open_dialog }, &tab),
        [&](Event e) {
            ...
            if (e == Event::Special("\x17") && tab == 5) { do_save_cfg(); return true; }
            ...
            if (cmd_mode) {
                ...
                if (e == Event::Return)    {
                    ...
                    std::string edit_path    = edit    ? cmd_buf.substr(3) : "";
                    ...
                    if (save_as && !save_as_path.empty()) do_write(resolve(save_as_path));
                    if (edit    && !edit_path.empty())    load_path(resolve(edit_path));
                    return true;
                }
